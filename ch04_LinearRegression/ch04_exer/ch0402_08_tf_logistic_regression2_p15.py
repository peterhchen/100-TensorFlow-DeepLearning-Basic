# TensorFlow APIs IN THIS EXAMPLE: tf.nn.sigmoid(tf.matmul(...)); tf.Variable(tf.random_normal(...))
# tf.placeholder(...); tf.pow(model_y-Y, 2)/(2); tf.global_variables_initializer; tf.initialize_all_variables().run() 
import tensorflow as tf; import numpy as np; from sklearn.utils import shuffle; np.random.seed(0) # generate same random #s
import matplotlib.pyplot as plt; trainsamples = 200; testsamples = 60
...
#print("dsX:",dsX); print("dsY:",dsY)
print("dsX shape:",dsX.shape); print("dsY shape:",dsY.shape)
X = tf.placeholder("float"); Y = tf.placeholder("float")
# Create first hidden layer
hw1 = tf.Variable(tf.random_normal([1, 10], stddev=0.1))
# Create output connection
ow = tf.Variable(tf.random_normal([10, 1], stddev=0.1)); print("output ow:",ow)
# Create bias
b = tf.Variable(tf.random_normal([10], stddev=0.1)); print("bias b:",b); model_y = model(X, hw1, b, ow)
# Cost function
...
with tf.Session() as sess:
 ...
 #tf.global_variables_initializer()   #initialize all variables
  print("-------------------------"); print("bias b:"); print(sess.run(b));print("output ow:"); print(sess.run(ow)); print("-------------------------\n")
...
plt.figure(); plt.title('Original data'); plt.scatter(dsX,dsY); plt.show()